(function() {
  var hasPerformance = window.performance && window.performance.now && typeof window.performance.now != 'object';
  var PerfMeter = function () {
    this.started = {};
    this.finished = {};
  };
  PerfMeter.prototype.start = function(label) {
    this.started[label] = hasPerformance ? window.performance.now() : +new Date();
  };
  PerfMeter.prototype.end = function(label) {
    var now = hasPerformance ? window.performance.now() : +new Date();
  ...
  PerfMeter.prototype.print = function(groupName, var_args) {
    ...
    if (console && typeof console.log != 'object' && typeof console.group != 'object') {
