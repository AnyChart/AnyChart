 ...
 * @private
 */
anychart.math.dmi.calculateDIValues_ = function(context, prevClose, highValue, prevHigh, lowValue, prevLow, pdiSumSubtrahend, ndiSumSubtrahend, trSumSubtrahend) {
  var dTmpP, dTmpN, TR;
 ...
 * @private
 */
anychart.math.dmi.calculateResultDIDX_ = function(context) {
  ...
  if (highQueue.getLength() < (context.period + 1)) {
    ...
    if (isNaN(context.pdiValue)) {
      ...
      for (i = 1; i < context.period + 1; i++) {
        ...
        anychart.math.dmi.calculateDIValues_(context, prevClose, highValue, prevHigh, lowValue, prevLow, 0, 0, 0);
      }
      ...
      anychart.math.dmi.calculateDIValues_(context, prevClose, highValue, prevHigh, lowValue, prevLow, context.pdiSumValue / context.period, context.ndiSumValue / context.period, context.trSumValue / context.period);
    }

    rv = anychart.math.dmi.calculateResultDIDX_(context);
    resultPDI = rv[0];
    resultNDI = rv[1];
    dx = rv[2];
