    ...
    if (!isNaN(newActualStart)) {
      ...
      switch (el.tag.type) {
        ...
        case anychart.enums.TLElementTypes.PERIODS:
          var interEvent = this.getInteractivityEvent(e);
          if(interEvent) {
            var destinationItem = interEvent['item'];
            periodIndex = el.tag.periodIndex;
            periodStart = /** @type {number} */ (dataItem.getMeta(anychart.enums.GanttDataFields.PERIODS, periodIndex, anychart.enums.GanttDataFields.START));
 ...
 * @param {boolean} considerBaseline - Whether to consider baseline appearance for parent task. Fixes DVF-4652.
 * @private
 */
anychart.ganttModule.TimeLine.prototype.iterateChildMilestones_ = function(
  ...
  if (depthMatches || depthBaselineMatches) {
    ...
    var milestonePreviewTop = considerBaseline && above ? totalTop + halfHeight : totalTop;
