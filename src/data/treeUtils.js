goog.provide('anychart.data.treeUtils');
 ...
 * @private
 */
anychart.data.treeUtils.ToCsvContext_;



/**
 * Creates data suitable to create csv.
 * @param {anychart.data.treeUtils.ToCsvContext_} context
 ...
 * @private
 */
anychart.data.treeUtils.makeObject_ = function(context, node, rawData, headers, headersLength, parentId, originalParent) {
  ...
  if (children && children.length) {
    for (var i = 0, len = children.length; i < len; i++)
      anychart.data.treeUtils.makeObject_(context, children[i], rawData, headers, headersLength, parentId, data['id']);
  }
};
 ...
 * @param {anychart.data.Tree|anychart.data.TreeView} data
 * @param {Object.<string, (string|boolean|undefined)>=} opt_csvSettings CSV settings.
 * @return {string} CSV string.
 */
anychart.data.treeUtils.toCsv = function(data, opt_csvSettings) {
  if (!data) return '';
  ...
  for (i = 0; i < roots.length; i++) {
    anychart.data.treeUtils.makeObject_(context, roots[i], rawData, headers, 2, null, null);
  }
